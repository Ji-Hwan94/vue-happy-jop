                    ...
                    <tr style="border: 0px; border-color: blue">
                        <td width="50" height="25" style="font-size: 100%; text-align: left">
                            <div id="searchArea" class="d-flex justify-content-around mb-2 mt-2">
                                ...
                                <input type="text" style="width: 200px" class="form-control" v-model="sname" />
                                <input type="date" style="width: 15%" class="form-control" v-model="fromDate" />
                                ~
                                <input type="date" style="width: 15%" class="form-control" v-model="toDate" />
                                <span class="fr">
                                    <a
                                        @click="searchList()"
                                    ...
                                    <a class="btn btn-primary mx-2" @click="newReg()" name="modal">
                                        <span>신규등록</span>
                                    </a>
                                </span>
                            </div>
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item" :key="1">
                    <button type="button" class="page-link" v-if="currentBlock > 1" @click="clickPrevPage">이전</button>
                </li>
                <template v-for="(pageNumber, index) in Array(blockSize).fill(startPage)">
                    <li
                        :class="pageNumber + index == currentPage ? 'pageItem active' : 'pageItem'"
                        v-if="pageNumber + index <= totalPage"
                        :key="pageNumber"
                    >
                        <button class="page-link" @click="clickPageNumber(pageNumber + index)">
                            {{ pageNumber + index }}
                        </button>
                    </li>
                </template>
                <li class="page-item" :key="totalPage + 1">
                    <button type="button" class="page-link" v-if="currentBlock != lastBlock" @click="clickNextPage">
                        다음
                    </button>
                </li>
            </ul>
...
export default {
    ...
    methods: {
        searchList() {
            ...
            this.axios
                .post('/api/notice/noticelistjson.do', params)
                .then((response) => {
                    this.items = response.data.listdata;
                    this.totalCount = response.data.listcnt;
                })
